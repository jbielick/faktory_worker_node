...
`);
  ...
  handshake(conn, greeting) {
    ...
    return conn.sendWithAssert(
      [
        'HELLO',
        Client.encode(this.buildHello(greeting))
      ],
      'OK'
    );
  }
  ...
  async beat() {
    heartDebug('BEAT');
    const response = await this.send(['BEAT', Client.encode({ wid: this.wid })]);
    if (response[0] === '{') {
  ...
  async push(job) {
    ...
    await this.sendWithAssert(['PUSH', Client.encode(payloadWithDefaults)], 'OK');
   ...
   */
  async ack(jid) {
    return this.sendWithAssert(['ACK', Client.encode({ jid })], 'OK');
  }
