...
const START_DELAY = process.env.NODE_ENV === 'test' ? 1 : 50;
...
class Worker {
  ...
  async tick(pid) {
    ...
    try {
      ...
      console.error(e);
      setTimeout(() => this.setTick(pid), 2000);
    }
  }
  ...
  async work() {
    ...
    this.trapSignals();
    const starts = Array(this.concurrency)
      .fill(0)
      .map((_, index) => new Promise(resolve => (
        setTimeout(() => {
          this.setTick(`p${index}`);
          resolve();
        }, index * START_DELAY)
      )));
  ...
  fetch() {
    return this.clients.use(client => client.fetch(...this.queues));
  }
